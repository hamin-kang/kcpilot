...
def normalize_inline(text: object) -> str:
    """짧은 인라인 문자열(frontmatter 값·표 셀)용 정규화.

    문자 정규화 후 개행을 제거하고 양끝 공백을 정리한다. 줄 단위 노이즈 제거
    (개정마커·삭제줄)는 본문 전용이라 여기서는 하지 않는다.
    """
    return normalize_chars(str(text or "")).replace("\n", "").strip()


def normalize_chars(text: str) -> str:
    """문자 표기를 정규화한다(개행 보존). 본문·메타 정규화의 공통 기반.

    parse_laws.py·parse_emc_pdf.py의 메타데이터 정규화도 이 함수로 통일한다.
    """
    text = unicodedata.normalize("NFC", text)         # 자모 분해(NFD) → 조합형(NFC)
    text = text.replace("\u3000", " ")             # 전각 공백 → 일반 공백
    # 가운뎃점(·) 변종 통일: 자모영역 문자(ㆍ), 유니코드 변종(‧), PDF 추출 오류(ž)
    text = text.replace("ㆍ", "·").replace("‧", "·").replace("ž", "·")
    return text
...
def clean_legal_text(text: str) -> str:
    ...
    text = "\n".join(kept)
    text = normalize_chars(text)              # ④ 특수문자 정규화
