...
- **ai-service/** — FastAPI, Python 3.11, LangChain/LangGraph, Google Gemini, pgvector

## 서비스 간 관계

- `frontend` (Next.js) → `backend` (Spring Boot, 8080) → PostgreSQL
- `backend` → `ai-service` (FastAPI): KC 인증 사전진단 워크플로우 실행 위임
- `ai-service` → PostgreSQL (pgvector): RAG용 임베딩·법령 텍스트 검색 (관계형 DB와 동일 인스턴스 공유). 컬렉션 2개 — `kc_legal`(법령 조문) / `kc_items`(별표 품목, cert_level은 metadata)
- `ai-service`는 LangGraph 기반 워크플로우 (`main.py`의 `/ai/run-assessment`): classify → retrieve → match_items → diagnose → recall → critique 6노드. cert_level은 품목 분류표(kc_items) 매칭의 metadata에서 권위 있게 가져오고, 법령 본문 추론으로 새로 만들지 않는다
- `ai-service` → Vertex AI (Gemini): 임베딩·챗 호출. 인증은 GCP ADC(API 키 아님)
...
- ai-service LangGraph 워크플로우(`/ai/run-assessment`)는 연결 완료(walking skeleton, 시나리오 A 검증). 다음은 신뢰도 임계값 정답셋 보정(`docs/requirements.md` §4.2.1 Phase 3).
